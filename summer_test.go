...
func TestThrowsNiceErrorOnAttemptingNilInjection(t *testing.T) {
	type simpleStruct struct{}

	var nilStruct *simpleStruct
	container := NewContainer()
	container.Add(nilStruct, "")
	err := container.PerformInjections()

	if err.Error() != "Summer: Attempted to inject into a nil struct (structs must be allocated before injection!)" {
		t.Fail()
	}
}
